...
- **`-O2` sensitivity in `fs/buffer.c`, `fs/bitmap.c` and `kernel/vsprintf.c`.** We compile `fs/buffer.o`, `fs/bitmap.o` and `kernel/vsprintf.o` at `-O1` as a conservative shield. The isolated reproductions in `tests/compiler-cases/` show that `bitmap_inline_asm` fails at `-O2` on x86_64 because the original inline-asm bit macros lack a `"memory"` clobber (undefined behavior in the GCC contract, not a compiler bug). The `buffer_freelist` and `vsprintf_percent_s` cases could not be reproduced in isolation on GCC 13.3; their historical `-O2` symptoms are recorded as unproven hypotheses. See `tests/compiler-cases/CLASSIFICATION.md`, `docs/AUDIT-STATEMENTS.md` and Waves 075–083.
